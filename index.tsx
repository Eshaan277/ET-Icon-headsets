...
export default function Home() {
  ...
  return (
    <>
      ...
      <section id="specs-section" style={{ padding:'120px 48px',background:'#080808',borderTop:'1px solid var(--border)',position:'relative',zIndex:10 }}>
        ...
        <div style={{ maxWidth:1200,margin:'0 auto',display:'grid',gridTemplateColumns:'1fr 1fr',gap:80,alignItems:'center' }}>
          <div style={{ position:'relative',height:500,display:'flex',alignItems:'center',justifyContent:'center' }}>
            ...
            <img src={IMG_ASSEMBLED} alt="SOOS Pro X1" style={{ width:'100%',maxWidth:360,height:'auto',filter:'drop-shadow(0 20px 60px rgba(200,169,110,.22))',animation:'floatY 4s ease-in-out infinite',position:'relative',zIndex:2 }} />
          </div>
          <div style={{ display:'flex',flexDirection:'column' }}>
            {SPECS.map(([label,val,unit],i) => (
              <div key={i} className="spec-row">
                <div style={{ fontFamily:"'Space Mono',monospace",fontSize:11,letterSpacing:2,textTransform:'uppercase',color:'var(--dim)' }}>{label}</div>
                <div style={{ fontFamily:"'Syne',sans-serif",fontSize:18,fontWeight:800,textAlign:'right' }}>
                  {val}{unit && <span style={{ fontSize:11,fontWeight:400,color:'var(--dim)',marginLeft:4 }}>{unit}</span>}
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ============ QUOTE ============ */}
      <section style={{ padding:'160px 48px',textAlign:'center',background:'var(--bg)',borderTop:'1px solid var(--border)',position:'relative',zIndex:10 }}>
        <div style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:'clamp(32px,5vw,68px)',letterSpacing:3,lineHeight:1.15,maxWidth:900,margin:'0 auto 48px' }}>
          "Not just headphones.<br />A <span style={{ color:'var(--accent)' }}>dimensional portal</span> for sound.<br />Engineering that borders on the spiritual."
        </div>
        <div style={{ fontFamily:"'Space Mono',monospace",fontSize:10,letterSpacing:4,textTransform:'uppercase',color:'var(--dim)' }}>— The Audiophile Review · 10/10 · Product of the Year 2025</div>
        <div style={{ display:'flex',gap:60,justifyContent:'center',marginTop:64,flexWrap:'wrap' }}>
          {[['4.9','Sound Score'],['98%','Recommend Rate'],['#1','Premium Wireless 2025'],['6×','Award Winner']].map(([v,l]) => (
            <div key={l} style={{ textAlign:'center' }}>
              <div style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:60,color:'var(--accent)',lineHeight:1 }}>{v}</div>
              <div style={{ fontFamily:"'Space Mono',monospace",fontSize:9,letterSpacing:3,textTransform:'uppercase',color:'var(--dim)',marginTop:8 }}>{l}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* ============ COLORS ============ */}
      <section id="colors-section" style={{ padding:'120px 48px',background:'#080808',borderTop:'1px solid var(--border)',position:'relative',zIndex:10 }}>
        <div style={{ textAlign:'center',marginBottom:80 }}>
          <div style={{ fontFamily:"'Space Mono',monospace",fontSize:10,letterSpacing:5,textTransform:'uppercase',color:'var(--accent)',marginBottom:16 }}>Color Editions</div>
          <div style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:'clamp(48px,6vw,80px)',letterSpacing:3,lineHeight:1 }}>Your Voice.<br />Your Finish.</div>
        </div>
        <div style={{ display:'flex',gap:32,justifyContent:'center',flexWrap:'wrap' }}>
          {COLORS.map((c,i) => (
            <div key={i} className={`color-option${selectedColor===i?' selected':''}`} onClick={() => { setSelectedColor(i); toast(`${c.label} Selected`); }}>
              <div className="color-swatch" style={{ background:c.bg, ...(c.shadow?{boxShadow:c.shadow}:{}) }} />
              <div className="color-name">{c.name}</div>
            </div>
          ))}
        </div>
        <div style={{ textAlign:'center',marginTop:40,fontFamily:"'Space Mono',monospace",fontSize:10,letterSpacing:4,textTransform:'uppercase',color:'var(--accent)' }}>
          Selected: {COLORS[selectedColor].label}
        </div>
      </section>

      {/* ============ CTA ============ */}
      <section id="cta-section" style={{ padding:'160px 48px',background:'var(--bg)',borderTop:'1px solid var(--border)',textAlign:'center',position:'relative',overflow:'hidden',zIndex:10 }}>
        <div style={{ position:'absolute',fontFamily:"'Bebas Neue',sans-serif",fontSize:'20vw',color:'rgba(200,169,110,.03)',top:'50%',left:'50%',transform:'translate(-50%,-50%)',whiteSpace:'nowrap',pointerEvents:'none',letterSpacing:20 }}>SOOS</div>
        <div style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:'clamp(60px,9vw,120px)',letterSpacing:4,lineHeight:.88,marginBottom:32,position:'relative',zIndex:1 }}>Sound Has<br />No Limits</div>
        <div style={{ fontFamily:"'Syne',sans-serif",fontSize:16,color:'var(--dim)',marginBottom:56,maxWidth:500,marginLeft:'auto',marginRight:'auto',position:'relative',zIndex:1 }}>Experience the future of personal audio. Free shipping. 60-day returns. Lifetime warranty on drivers.</div>
        <div style={{ display:'flex',gap:16,justifyContent:'center',position:'relative',zIndex:1,flexWrap:'wrap' }}>
          <button className="btn-primary" onClick={() => setModalOpen(true)}><span>Order ET Icon Headsets — $449</span></button>
          <button className="btn-secondary" onClick={() => toast('Tech Spec PDF Downloading...')}>View Tech Specs</button>
        </div>
        <div style={{ display:'flex',gap:40,justifyContent:'center',marginTop:56,flexWrap:'wrap',position:'relative',zIndex:1 }}>
          {['Free Worldwide Shipping','60-Day Returns','Lifetime Driver Warranty','Secure Checkout'].map(item => (
            <div key={item} style={{ display:'flex',alignItems:'center',gap:10,fontFamily:"'Space Mono',monospace",fontSize:10,letterSpacing:2,textTransform:'uppercase',color:'var(--dim)' }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}><path d="M5 13l4 4L19 7"/></svg>
              {item}
            </div>
          ))}
        </div>
      ...
      {/* ============ FOOTER ============ */}
      <footer style={{ borderTop:'1px solid var(--border)',padding:'60px 48px 80px',background:'#060606',position:'relative',zIndex:10 }}>
        <div style={{ display:'grid',gridTemplateColumns:'2fr 1fr 1fr 1fr',gap:48,maxWidth:1200,margin:'0 auto 60px' }}>
          <div>
            <div style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:36,letterSpacing:6,marginBottom:16 }}>SOOS</div>
            <div style={{ fontFamily:"'Syne',sans-serif",fontSize:13,color:'var(--dim)',lineHeight:1.8,maxWidth:260 }}>We believe sound is the most intimate form of human experience. ET Icon Headsets exist to make that experience boundless, immersive, and perfectly personal.</div>
            <div style={{ display:'flex',gap:12,marginTop:24 }}>
              {[
                { label:'IG', icon:<><rect x="2" y="2" width="20" height="20" rx="5"/><circle cx="12" cy="12" r="5"/><circle cx="17.5" cy="6.5" r="1" fill="currentColor"/></> },
                { label:'TW', icon:<path d="M22 4s-2.7 1-4 1.2C16.9 4 15.6 3 14 3c-2.8 0-5 2.2-5 5 0 .4 0 .7.1 1C6 8.8 3.4 7.4 1.6 5.2c-.4.7-.6 1.5-.6 2.3 0 1.7.9 3.2 2.2 4.1-.8 0-1.6-.2-2.2-.6v.1c0 2.4 1.7 4.4 4 4.8-.4.1-.8.2-1.3.2-.3 0-.6 0-.9-.1.6 2 2.4 3.4 4.6 3.5C5.7 20.6 3.9 21.3 2 21.3c-.3 0-.7 0-1-.1C3.2 22.4 5.5 23 8 23c9.5 0 14.7-7.9 14.7-14.7v-.7C23.8 6.8 25 5.5 25 4z"/> },
                { label:'YT', icon:<><rect x="2" y="6" width="20" height="13" rx="3"/><polygon points="10,9.5 16,12.5 10,15.5" fill="currentColor" stroke="none"/></> },
              ].map((s) => (
                <button key={s.label} onClick={() => toast(`Opening ${s.label}...`)} style={{ width:36,height:36,border:'1px solid var(--border)',background:'transparent',cursor:'pointer',color:'var(--dim)',display:'flex',alignItems:'center',justifyContent:'center',transition:'border-color .3s,color .3s' }} onMouseOver={e=>{ (e.currentTarget as HTMLButtonElement).style.borderColor='var(--accent)'; (e.currentTarget as HTMLButtonElement).style.color='var(--accent)'; }} onMouseOut={e=>{ (e.currentTarget as HTMLButtonElement).style.borderColor='var(--border)'; (e.currentTarget as HTMLButtonElement).style.color='var(--dim)'; }}>
          ...
          {[
            { h:'Products', links:[['Pro X1','You are looking at it!'],['Studio Edition','Coming 2026'],['Sport Wireless','Coming 2026'],['In-Ear Pro','Coming Soon'],['Accessories','Browse accessories']] },
            { h:'Company', links:[['About','About SOOS Audio'],['Our Story','The founding story'],['Careers','12 open roles'],['Press','Press kit ready'],['Awards','6 major awards']] },
            { h:'Support', links:[['FAQ','Frequently asked questions'],['Warranty','Lifetime driver warranty'],['Contact Us','open-modal'],['Firmware','v3.2.1 is current'],['SOOS App','iOS & Android']] },
          ].map((col) => (
            <div key={col.h}>
              <h4 style={{ fontFamily:"'Space Mono',monospace",fontSize:10,letterSpacing:3,textTransform:'uppercase',color:'var(--accent)',marginBottom:24 }}>{col.h}</h4>
              <ul style={{ listStyle:'none',display:'flex',flexDirection:'column',gap:12 }}>
                {col.links.map(([label,msg]) => (
                  <li key={label}><button className="footer-link" onClick={() => { if(msg==='open-modal') setModalOpen(true); else toast(msg); }}>{label}</button></li>
                ))}
              </ul>
            </div>
          ))}
        </div>
        <div style={{ borderTop:'1px solid var(--border)',paddingTop:24,display:'flex',justifyContent:'space-between',alignItems:'center',maxWidth:1200,margin:'0 auto' }}>
          <div style={{ fontFamily:"'Space Mono',monospace",fontSize:10,letterSpacing:2,color:'var(--dim)' }}>© 2025 ET Icon Headsets. All rights reserved.</div>
          <div style={{ display:'flex',gap:24 }}>
            {['Privacy','Terms','Cookies'].map(p => (
              <button key={p} className="footer-link" style={{ fontSize:9,letterSpacing:2,textTransform:'uppercase' }} onClick={() => toast(`${p} Policy`)}>{p}</button>
            ))}
          </div>
        </div>
      </footer>

      <Modal open={modalOpen} onClose={() => setModalOpen(false)} onSubmit={submitOrder} />
      <Toast ref={toastRef} />
    </>
  );
}
